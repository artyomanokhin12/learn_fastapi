...
class HotelDAO(BaseDAO):
    ...
    @classmethod
    async def hotels_list(cls, location: str, date_from: date, date_to: date):
        booked_rooms = (select(Bookings.room_id, func.count(Bookings.room_id).label('rooms_booked')).select_from(Bookings).where(
            or_(
                and_(
